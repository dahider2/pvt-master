...
<!-- //language-select -->
<link rel="stylesheet" href="{{asset('css/flexslider.css')}}" media="screen" /><!-- flexslider css -->
</head>
<body>
	<!-- Navigation -->
          ...
          {{$single_data[0]->title}}
            ...
            <ul class="slides">
              @foreach($single_data as $data)
    							<li data-thumb="{{asset('storage/upload/'.$data->filename)}}">
    								<img src="{{asset('storage/upload/'.$data->filename)}}" />
    							</li>
    							...
    							<li data-thumb="{{asset('images/ss4.jpg')}}">
    								<img src="{{asset('images/ss4.jpg')}}" alt="not image loaded" />
    							</li> -->
              @endforeach
						</ul>
					</div>
